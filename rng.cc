...
double RNG::gamma(double shape, double scale) {
  if (shape < 1.0) {
    return gamma(shape + 1.0, scale) * pow(rand_open01(), 1.0 / shape);
  }
...
int RNG::poisson(double mu) {
  ...
  if (!(big_mu && mu == muprev)) {
    if (big_mu) {
      ...
      if (mu != muprev) {
        muprev = mu;
        m = static_cast<int>((1.0 < mu) ? mu : 1.0);
...
int RNG::binomial(double pp, int n) {
  ...
  const double p = (pp < 1.0 - pp) ? pp : 1.0 - pp;
  ...
  for (;;) {
    ...
    if (k <= 20 || k >= npq / 2 - 1) {
      ...
      amaxp = (k / npq) * ((k * (k / 3.0 + 0.625) + 0.1666666666666) /
               npq + 0.5);
      ynorm = -k * k / (2.0 * npq);
      alv = log(v);
      if (alv < ynorm - amaxp) {
